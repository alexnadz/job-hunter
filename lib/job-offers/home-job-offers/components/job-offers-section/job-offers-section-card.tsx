import { FilteredJobOffer, getFormattedSalary } from '@/lib/job-offers/shared';

// type JobType = 'Full-time' | 'Part-time' | 'Contract' | 'Remote';

type JobOffersSectionCardProps = {
    jobOffer: FilteredJobOffer;
    // title: string;
    // company: string;
    // location: string;
    // salary: string;
...
// TODO: to finish
export const JobOffersSectionCard = ({ jobOffer }: JobOffersSectionCardProps) => {
    const { title, salary_min, salary_max, employers } = jobOffer;
    ...
    return (
        <div className="rounded-lg border bg-card p-6 shadow-sm transition-all hover:shadow-md">
            ...
            <div className="flex items-center justify-between">
                <span className="font-medium text-blue-600">{`Salary: ${getFormattedSalary(salary_min, salary_max)}`}</span>
            </div>
        </div>
    );
};
